   ...
   */
   ...
   public boolean isPath(E fromKey, E toKey)
   {
      ...
      if (isEmpty())
         return false;
      Vertex start = first;
      while (start != null && cmp.compare(fromKey, start.data) != 0)
         start = start.pNextVertex;
      if (start == null)
         return false;
      Vertex end = first;
      while (end != null && cmp.compare(toKey, end.data) != 0)
         end = end.pNextVertex;
      if (end == null)
         return false;
      if (start == end)
      ...
      while (!queue.isEmpty())
      {
         ...
         if (tmp == end)
            return true;
         Edge edgeWalk = tmp.pEdge;
         while (edgeWalk != null)
         {
            Vertex dest = edgeWalk.destination;
            if (dest.processed == 0)
            {
               dest.processed = 1;
               queue.add(dest);
