 ...
 * @author Duncan, Quentin Bordelon
 * <pre>
...
public class GraphDemo
{
      ...
      while (menuReturnValue != 0)
      {
         ...
         switch(menuReturnValue)
         {
             ...
             case 2: //Check if a digraph is strongly connected
                ...
                boolean strong = isStronglyConnected(g);
                ...
                break;                 
            case 3://Single-Source Shortest-path algorithm
                ...
                if (g.isPath(new City(initial), new City(dest)) && g.isPath(new City(dest), new City(initial)))
                {
                  ...
                  int[] pred = new int[n + 1];
                   ...
                   String srcLabel  = g.retrieveVertex(new City(initial)).getLabel().trim();
                   String destLabel = g.retrieveVertex(new City(dest)).getLabel().trim();
 
                   bellman(g, dist, pred, initial, dest);
 
                   System.out.printf("%s -> %s:%n", srcLabel, destLabel);
                   List<Integer> path = new ArrayList<>();
                   for (int cur = dest; cur != -1; cur = pred[cur])
                      path.add(cur);
                   ...
                   for (int k = 0; k < path.size() - 1; k++)
                   {
                      int from = path.get(k);
                      int to = path.get(k + 1);
                      double w = g.retrieveEdge(new City(from), new City(to));
                      String fromLabel = g.retrieveVertex(new City(from)).getLabel().trim();
                      String toLabel = g.retrieveVertex(new City(to)).getLabel().trim();
                      System.out.printf("%-22s ->   %-26s%.2f mi%n", fromLabel, toLabel, w);
                   }
                   System.out.printf("Distance: %.2f mi%n%n", dist[dest]);
 
                   dist = new double[n + 1];
                   pred = new int[n + 1];
                   bellman(g, dist, pred, dest, initial);
 
                   System.out.printf("%s -> %s:%n", destLabel, srcLabel);
                   path = new ArrayList<>();
                   for (int cur = initial; cur != -1; cur = pred[cur])
                      path.add(cur);
                   Collections.reverse(path);
                   for (int k = 0; k < path.size() - 1; k++)
                   {
                   ...
                   System.out.printf("Distance: %.2f mi%n", dist[initial]);
                   System.out.println("=========================================================================================");
                   System.out.printf("Round Trip Distance: %.2f mi%n%n", dist[dest] + dist[initial]);
   ...
   private static int menu()
   {
      ...
      do
      {
         ...
         System.out.println("[3] Bellman-Ford's Shortest Round Trip in G");
   ...
   /**
    * This method generates a single source shortest path tree using the 
	* Bellman-Ford single-source shorted path algorithm
    * @param g a weighted directed graph
    * @param dist the distance array from the source 
	* @param pred the parent array representation of the shortest path tree 
	* @param source the source vertex
	* @param destination the destination vertex
    * @return true if this graph has no negative cycle; otherwise, false 
    * @throws GraphException when this graph is empty
    */    
   private static boolean bellman(Graph<City> g, double[] dist, int[] pred, int source, int destination) throws GraphException
   {
      /**
	  ...
	  //Define a dynamic an array of EdgeTypes to store edges in 
	  //the graph.
     ...
     for (int i = 1; i <= n; ++i) {
      for (int j = 1; j <= n; ++j) {
         if (g.isEdge(new City(i), new City(j))) {
            ...
            e.source = i;
            e.destination = j;
            e.weight = g.retrieveEdge(new City(i), new City(j));
            edges.add(e);
         }
      }
     }
     ...
     for (int i = 1; i < n; ++i) {
      for (EdgeType e : edges) {
         if (dist[e.source] != INFINITY && dist[e.source] + e.weight < dist[e.destination]) {
     ...
     for (EdgeType e : edges) {
      if (dist[e.source] != INFINITY && dist[e.source] + e.weight < dist[e.destination]) return false;
     }
  ...
  /**
   ...
   private static int[] topSortInDeg(Graph<City> g) throws GraphException
   {
      ...
      for (int i = 1; i <= n; ++i) {
         inDeg[i] = (int) g.inDegree(new City(i));
      }
      ...
      while (!pq.isEmpty()) {
         ...
         for (int j = 1; j <= n; ++j) {
            if (g.isEdge(new City(v), new City(j))) {
               inDeg[j]--;
               if (inDeg[j] == 0) {
                  pq.add(j);
               }
   ...
   public static boolean isStronglyConnected(Graph g)
   {
       ...
       for (int i = 1; i <= n; ++i) {
         for (int j = 1; j <= n; ++j) {
            if (i != j) {
               if (!g.isPath(new City(i), new City(j))) return false;
            }
         }
       }
